...
First select each day's own cheapest `dailyOnMinutes`. At recalculation time, treat selected today periods that have already started as committed, including the currently active period; only selected today periods whose start is at or after the recalculation instant may be moved. Then replace the most expensive movable today minutes with additional, unselected tomorrow capacity only while that tomorrow capacity is strictly cheaper. The mode may make the remaining part of today shorter and tomorrow longer, but it must not move tomorrow's base quota into today when today is cheaper. Break equal-price ties by earlier timestamp so shifted capacity occurs as early as possible and equal prices do not unnecessarily move today into tomorrow. If tomorrow prices are incomplete, use the strict daily calculation. Existing `CHEAPEST_HOURS` semantics must not change.
...
- `MainLayout` owns the Windows 98 desktop, shortcuts, Start menu, bottom taskbar, and multiple routed feature windows. Keep feature views under `layout = MainLayout.class` so their existing routes and deep links open inside the desktop. MainLayout retains open route components in memory until closed; opening the same route through navigation replaces its old window, while Start and taskbar switching preserve its form state. Do not restore the old `Windows95Header` top navigation.
- Windows can be dragged by their title bars within the desktop; positions are not persisted. File/View/Help menus live in each window, with no address bar. The Start menu has a vertical Pörssiohjain 98 brand strip and pixel SVG icons under `META-INF/resources/icons/desktop`. The window's minimize control hides its content until the taskbar button restores it; maximize fills the space above the taskbar; close returns to `/desktop`. New feature windows open maximized. Desktop shortcuts fill the available space on all screen sizes; overflow shortcuts are hidden and remain available in Start. On screens at most 620px wide, the feature window fills the available screen.
- Register feature navigation through the shared Start and desktop shortcut list. Logout also has a red desktop shortcut. Google Play and Buy Me a Coffee are external links in both the desktop and Start. Active service notices appear in a pale-yellow banner above the icons on all screen sizes and can also be opened from Start. Restore the session locale before constructing desktop labels. Respect effective-account admin visibility and impersonation state. Keep logout and language switching available from Start.
- The desktop device-status widget uses the effective account’s owned and shared devices, counts API or MQTT connectivity once per device, and refreshes every minute. It is a compact retro button to the right of shortcuts on all screen sizes; clicking the whole widget opens My devices.
