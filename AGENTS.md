...
- Windows can be dragged by their title bars within the desktop; positions are not persisted. File/View/Help menus live in each window, with no address bar. The Start menu has a vertical Pörssiohjain 98 brand strip and pixel SVG icons under `META-INF/resources/icons/desktop`. The window's minimize control hides its content until the taskbar button restores it; maximize fills the space above the taskbar; close returns to `/desktop`. On screens at most 620px wide, desktop shortcuts are hidden and the feature window fills the available screen; navigation remains in Start.
