...
export default function WindForecastCard() {
  ...
  const innerWidth=CHART_WIDTH-CHART_PADDING_LEFT-CHART_PADDING_RIGHT, innerHeight=CHART_HEIGHT-CHART_PADDING_TOP-CHART_PADDING_BOTTOM;
  const yAxisValues=Array.from({length:Y_AXIS_STEPS+1},(_,index)=>max-(range*index)/Y_AXIS_STEPS);
  const timeLabels=getTimeLabelPoints(data.points,6);
  ...
  return <article className="app-card overflow-hidden">
    <div className="grid grid-cols-1 gap-6 p-4 sm:p-5 lg:grid-cols-[minmax(0,1fr)_17rem]">
      <div>
        ...
        {values.length ? <div className="rounded-3xl p-4 sm:p-5" style={{ background: "linear-gradient(180deg, rgb(var(--chart-panel-start)), rgb(var(--chart-panel-end)))" }}><div className="-mx-1 overflow-x-auto px-1 pb-2 sm:mx-0 sm:px-0"><svg className="h-auto min-w-[44rem] aspect-[4/1] w-full sm:min-w-0" viewBox={`0 0 ${CHART_WIDTH} ${CHART_HEIGHT}`} role="img" aria-label={t("aria")}><defs><linearGradient id="wind-forecast-fill" x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stopColor="rgb(var(--color-secondary) / 0.35)"/><stop offset="100%" stopColor="rgb(var(--color-secondary) / 0.02)"/></linearGradient></defs><rect fill="rgb(var(--chart-plot-background))" height={innerHeight} rx="18" width={innerWidth} x={CHART_PADDING_LEFT} y={CHART_PADDING_TOP}/>{dayBands.map((band,index)=><g key={band.dayKey}>{index%2===1?<rect fill="rgb(var(--color-primary) / 0.055)" height={innerHeight} width={Math.max(band.endX-band.startX,0)} x={band.startX} y={CHART_PADDING_TOP}/>:null}{index>0?<line stroke="rgb(var(--color-primary) / 0.42)" strokeDasharray="4 6" strokeWidth="1.5" x1={band.startX} x2={band.startX} y1={CHART_PADDING_TOP} y2={CHART_PADDING_TOP+innerHeight}/>:null}<text fill="rgb(var(--color-on-surface-variant))" fontSize="11" fontWeight="700" textAnchor="start" x={band.startX+8} y={CHART_PADDING_TOP+18}>{band.label}</text></g>)}{yAxisValues.map((value,index)=>{const y=CHART_PADDING_TOP+(innerHeight*index)/Y_AXIS_STEPS;return <g key={`${value}-${index}`}><line stroke="rgb(var(--color-outline-variant) / 0.6)" strokeDasharray="6 8" strokeWidth="1" x1={CHART_PADDING_LEFT} x2={CHART_PADDING_LEFT+innerWidth} y1={y} y2={y}/><text fill="rgb(var(--color-on-surface-variant))" fontSize="12" textAnchor="end" x={CHART_PADDING_LEFT-10} y={y+4}>{formatMw(value)}</text></g>})}{timeLabels.map((point)=>{const index=data.points.findIndex(candidate=>candidate.startTime===point.startTime);const x=getX(getTimestamp(point),getTimestamp(data.points[0]),getTimestamp(data.points[data.points.length-1]),innerWidth);return <g key={point.startTime}><line stroke="rgb(var(--color-outline-variant) / 0.45)" strokeWidth="1" x1={x} x2={x} y1={CHART_PADDING_TOP} y2={CHART_PADDING_TOP+innerHeight}/><text fill="rgb(var(--color-on-surface-variant))" fontSize="12" textAnchor={index===data.points.length-1?"end":index===0?"start":"middle"} x={x} y={CHART_PADDING_TOP+innerHeight+20}>{formatNordpoolTime(point.startTime,data.timezone)}</text></g>})}<path d={buildAreaPath(data.points,min,range,innerWidth,innerHeight)} fill="url(#wind-forecast-fill)" stroke="none"/><path d={buildLinePath(data.points,min,range,innerWidth,innerHeight)} fill="none" stroke="rgb(var(--color-primary))" strokeLinecap="round" strokeLinejoin="round" strokeWidth="4"/>{currentMarker?<><line stroke="rgb(var(--color-secondary) / 0.45)" strokeDasharray="5 7" strokeWidth="2" x1={currentMarker.x} x2={currentMarker.x} y1={CHART_PADDING_TOP} y2={CHART_PADDING_TOP+innerHeight}/><circle cx={currentMarker.x} cy={currentMarker.y} fill="rgb(var(--color-secondary-container))" r="7" stroke="rgb(var(--color-primary))" strokeWidth="3"/></>:null}</svg></div></div> : <p>{t("empty")}</p>}
      </div>
...
function WindDialog({open,close,timezone}:{open:boolean;close:()=>void;timezone:string}) {
 ...
 return <AppDialog isOpen={open} onClose={close} eyebrow={t("eyebrow")} title={t("title")} description={t("description")} maxWidthClassName="max-w-5xl">
  <div className="grid gap-5 lg:grid-cols-[1fr_22rem]">
   <div className="space-y-3">{items.length===0?<p className="rounded-xl bg-surface-container p-4 text-sm">{t("empty")}</p>:items.map(n=><article key={n.id} className="rounded-xl bg-surface-container p-4"><h4 className="font-headline text-lg font-bold">{n.name}</h4><p className="mt-1 text-sm text-on-surface-variant">{t(n.ruleType,{threshold:number(n.threshold,1)})}</p><div className="mt-3 flex gap-2"><button className="secondary-action px-3 py-2 text-xs" onClick={()=>edit(n)}>{common("edit")}</button><button className="secondary-action px-3 py-2 text-xs" onClick={async()=>{await deleteWindNotification(n.id);setItems(x=>x.filter(i=>i.id!==n.id))}}>{common("remove")}</button></div></article>)}</div>
