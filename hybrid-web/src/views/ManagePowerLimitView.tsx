...
export default function ManagePowerLimitView() {
  ...
  async function loadData() {
    ...
    try {
      ...
      setError(loadError instanceof Error ? loadError.message : "Failed to load power limit");
    } finally {
      setIsLoading(false);
    }
  }
  ...
  const handleSave = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setLimit(response);
      setMessage("Power limit saved.");
    } catch (saveError) {
      setError(saveError instanceof Error ? saveError.message : "Failed to save power limit");
    }
  };
  ...
  return (
    <>
      <PageHeader rightSlot={<Link className="secondary-action px-4 py-2 text-sm" to="/menu">Menu</Link>} title="Manage Power Limit" compact />
