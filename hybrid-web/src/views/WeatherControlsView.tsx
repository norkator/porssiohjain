      ...
      <main className="app-page pt-12">
        ...
        <section className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
          {!isLoading && controls.map((control) => (
            <article className="app-card border-l-4 border-secondary p-6" key={control.id}>
              <div className="mb-5 flex items-start justify-between gap-3">
                <span className="chip bg-surface-container-highest text-primary-container">{control.siteName ?? "No site"}</span>
              ...
              <Link className="secondary-action justify-center rounded-lg px-3 py-2 text-sm" to={`/weather-controls/${control.id}`}>Manage</Link>
            </article>
          ))}
        </section>

        <section className="mt-12 grid grid-cols-1 gap-4 sm:grid-cols-3">
          <div className="rounded-2xl bg-surface-container p-6"><p className="font-bold">Configured</p><p className="text-sm text-on-surface-variant">{controls.length} weather controls</p></div>
          <div className="rounded-2xl bg-surface-container p-6"><p className="font-bold">Sites</p><p className="text-sm text-on-surface-variant">{sites.length} available sites</p></div>
          <div className="rounded-2xl bg-surface-container p-6"><p className="font-bold">Last Update</p><p className="text-sm text-on-surface-variant">{latestUpdate ? formatDate(latestUpdate) : "No changes yet"}</p></div>
        </section>
      </main>
    </>
  );
