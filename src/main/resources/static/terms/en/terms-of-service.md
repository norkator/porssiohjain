By creating an account for Pörssiohjain, you agree to use the service responsibly and in accordance with applicable
laws.

## Development Status

- Pörssiohjain is heavily under development and should currently be considered a beta-stage service.
- Features, behavior, integrations, and user interface details may change at any time without prior notice.
- Bugs, incorrect behavior, incomplete functionality, and temporary service interruptions should be expected.

## Account Responsibility

- You are responsible for keeping your UUID and secret secure.
- You are responsible for actions taken using your account credentials.

## Service Availability

- The service is provided as is, with no guarantees of uptime, uninterrupted availability, stability, or continued access.
- Features may change, be updated, or be removed over time.
- No service-level commitment, uptime promise, or availability guarantee is provided at this stage.

## Automation Responsibility

- You are responsible for reviewing and validating any automations, control rules, or device integrations you configure.
- Pörssiohjain is not responsible for damage, cost, or loss caused by incorrect automation settings, device behavior, or third-party integrations.
- You use the service, all automations, and all related device controls entirely at your own risk.

## Updates

- These terms may be updated later.
- Continued use of the service may require accepting updated terms.
